...
Arguments = Location+StartDate+DataType+Limit

# # print Arguments

# #********************************Define data pull for up to 1000 records
...
#********************************** loop a data pull for more than 1000 records
def Write_CSV():
	with open('Stations.csv', 'wb') as f:
	    writer = csv.writer(f)
	    for aItem in TotalData:
	        writer.writerow(aItem)
...
def Process_To_List():
	...
	i = 0
	CheckDate = 0
	json_list = API_Download()
	while i<len(json_list):
		temp = json_list[i]['id']
		Record.append(temp)
		temp = json_list[i]['name']
		Record.append(temp)
		temp = json_list[i]['maxdate']
		var = copy.deepcopy(temp)
		var = datetime.datetime.strptime(temp, '%Y-%m-%d')
		if var<(datetime.datetime.strptime('2016-11-01', '%Y-%m-%d')):
			CheckDate = 1
		Record.append(temp)
		temp = json_list[i]['mindate']
		var = copy.deepcopy(temp)
		var = datetime.datetime.strptime(temp, '%Y-%m-%d')
		if var>(datetime.datetime.strptime('1950-10-01', '%Y-%m-%d')):
			CheckDate = 1
...
def API_Download():
	Arguments = Location+StartDate+DataType+Limit+Offset
...
def Create_Station_List():
	...
	while i==1000:
		...
		t = 0
		CheckDate = 0
		while t<len(json_list):
			temp = json_list[t]['id']
			Record.append(temp)
			temp = json_list[t]['name']
			Record.append(temp)
			temp = json_list[t]['maxdate']
			# var = copy.deepcopy(temp)
			# var = datetime.datetime.strptime(temp, '%Y-%m-%d')
			# if var<(datetime.datetime.strptime('2016-11-01', '%Y-%m-%d')):
			# 	CheckDate = 1
			Record.append(temp)
			temp = json_list[t]['mindate']
			# var = copy.deepcopy(temp)
			# var = datetime.datetime.strptime(temp, '%Y-%m-%d')
			# if var>(datetime.datetime.strptime('1950-10-01', '%Y-%m-%d')):
			# 	CheckDate = 1
